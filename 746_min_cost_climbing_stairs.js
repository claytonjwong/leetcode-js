...
let minCostClimbingStairs = A => {
    ...
    let go = i => {
        ...
        return A[i] + Math.min(go(i + 1), go(i + 2)); // cost of i-th stair plus min of (i + 1)-th and (i + 2)-th solution 🎯
    };
...
let minCostClimbingStairs = (A, m = {}) => {
    ...
    let go = i => {
        ...
        return m[i] = A[i] + Math.min(go(i + 1), go(i + 2)); // cost of i-th stair plus min of (i + 1)-th and (i + 2)-th solution 🎯
    };
    return Math.min(go(0), go(1)); // start on min(first step, second step)
};
...
let minCostClimbingStairs = A => {
    ...
    for (let i = 2; i < N; ++i)
        dp[i] = A[i] + Math.min(dp[i - 2], dp[i - 1]); // cost of i-th stair plus min of (i - 2)-th and (i - 1)-th solution 🎯
...
let minCostClimbingStairs = A => {
    ...
    for (let i = 2; i < N; ++i, a = b, b = c)
        c = A[i] + Math.min(a, b); // cost of i-th stair plus min of (i - 2)-th and (i - 1)-th solution 🎯
